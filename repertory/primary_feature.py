...
class Geometry(object):
    ...
    def Variable(self):
        ...
        if self.site[:3] == '101' or self.site[:3] == '100' or self.site[:3] == '001':
    ...
    def Site_1fold_111(self,folname,rutile_type): # second time repeating cell and calculate
        distance_list=[]
        if rutile_type == 'Ruo2':
            pathway='/p/project/lmcat/wenxu/jobs/opt_Ru/cal_1'
        if rutile_type == 'Iro2':
            pathway='/p/project/lmcat/wenxu/jobs/opt_Ir/cal_1'
        atoms=read('{}/{}O/opt.traj'.format(pathway,folname))
        atoms=atoms*(2,2,1)
        if self.site[:3] == '111':
            for  j in PM.site_1foldO_dic[self.site][1:]:
                distance_list.append(atoms.get_distance(PM.site_1foldO_dic[self.site][0],j))
            distance_list.sort()        
...
def Work_function(site,Ef,pathway,filename='xsf_ionic_and_hartree_potential'):
    ...
    list001=['001cuscusM','001bricusRu']
    if site in list110:
        number1=72; number2=77
    if site in list111 or site in list001:
...
def Dopant_feature(met,site,rutile_type):
    if site[-2:] == 'Ru':
        if rutile_type == 'Ruo2':
            State    =  PM.dopant_descriptor['Ru'][0]
            Enthalpy =  PM.dopant_descriptor['Ru'][1] 
            CR       =  PM.dopant_descriptor['Ru'][2]
            IR       =  PM.dopant_descriptor['Ru'][3]
            AR       =  PM.dopant_descriptor['Ru'][4]
        if rutile_type == 'Iro2':
            State    =  PM.dopant_descriptor['Ir'][0]
            Enthalpy =  PM.dopant_descriptor['Ir'][1] 
            CR       =  PM.dopant_descriptor['Ir'][2]
            IR       =  PM.dopant_descriptor['Ir'][3]
            AR       =  PM.dopant_descriptor['Ir'][4]
    if site[-1:] == 'M':
        State    =  PM.dopant_descriptor[met][0]
        Enthalpy =  PM.dopant_descriptor[met][1] 
        CR       =  PM.dopant_descriptor[met][2]
        IR       =  PM.dopant_descriptor[met][3]
        AR       =  PM.dopant_descriptor[met][4]
    return State, Enthalpy, CR, IR, AR
...
class DOS_extract(object):
    ...
    def D_band_kutosis(self):
        ...
        dbc=self.D_band_center()[0]
        dbw=self.D_band_width()
        d_band_mom4 = simps(sum_pdos*np.power(dos_energies-dbc,4),dos_energies)/simps(sum_pdos,dos_energies)
    ...
    def Magnetic_moment(self):
        ...
        if self.met == 'Ni' or self.met == 'Fe' or self.met == 'Co':
            up_dos=[]
            up_dos_energies=[]
            for d,e in zip (pdos[self.atom_index][states][0],dos_energies):
                if e < 0 :
                    up_dos.append(d)
                    up_dos_energies.append(e)
            up_filling = simps(up_dos,up_dos_energies)

            down_dos=[]
            down_dos_energies=[]

            for d,e in zip (pdos[self.atom_index][states][1],dos_energies):
                if e < 0 :
                    down_dos.append(d)
                    down_dos_energies.append(e)
            down_filling = simps(down_dos,down_dos_energies)

            mag_moment = abs(up_filling) - abs(down_filling)

        else:
            mag_moment = 0
        return round(mag_moment,4)
